...
function PetPhoto({ pet, onPhotoChange }: { pet: { nome: string; fotoUrl: string | null }; onPhotoChange: (url: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        className="relative h-32 w-32 cursor-pointer overflow-hidden rounded-full border-4 border-emerald-200 bg-slate-100"
      ...
      >
        {pet.fotoUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="currentColor" className="text-slate-300">
      ...
      <input
        ...
        className="hidden"
      />
      <p className="mt-2 text-xs text-slate-500">Clique para adicionar foto</p>
    </div>
  );
}
          ...
          <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-start">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <h1 className="mt-3 text-5xl font-black tracking-tight md:text-6xl">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-700">{pet.nome}</span>
              </h1>
              <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5 text-xs text-slate-500 sm:justify-start">
                <Link href="/atividades" className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 transition hover:bg-amber-100 hover:text-amber-700">
                  <span>🐕</span> {pet.raca}
                </Link>
                <Link href="/vida" className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 transition hover:bg-blue-100 hover:text-blue-700">
                  <span>📅</span> {idadeEmMeses < 12 ? `${idadeEmMeses}m` : `${Math.floor(idadeEmMeses / 12)}a ${idadeEmMeses % 12}m`}
                </Link>
                <Link href="/atividades" className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 transition hover:bg-pink-100 hover:text-pink-700">
                  <span>{pet.sexo === 'macho' ? '♂️' : '♀️'}</span> {pet.sexo === 'macho' ? 'Macho' : 'Fêmea'}
                </Link>
              </div>
              <div className="mt-4 flex flex-wrap justify-center gap-2 sm:justify-start">
                <Link href="/atividades" className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-700 transition hover:bg-emerald-100">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>
                  </svg>
                  Atividades
                </Link>
                <Link href="/racao" className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 transition hover:bg-amber-100">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <Link href="/vida" className="inline-flex items-center gap-1.5 rounded-full bg-purple-50 px-4 py-2 text-sm font-medium text-purple-700 transition hover:bg-purple-100">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
