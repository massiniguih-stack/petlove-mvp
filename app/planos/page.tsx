...
const funcionalidades = [
  ...
  { nome: 'Controle de peso avançado com gráficos', gratis: false, premium: true },
  { nome: 'Plano alimentar personalizado', gratis: false, premium: true },
  { nome: 'Plano de exercícios completo', gratis: false, premium: true },
  { nome: 'Alertas de vacinas e consultas', gratis: false, premium: true },
  { nome: 'Relatórios de saúde', gratis: false, premium: true },
  { nome: 'Comparação entre pets', gratis: false, premium: true },
  { nome: 'Suporte prioritário', gratis: false, premium: true },
  { nome: 'Sem anúncios', gratis: false, premium: true },
];

export default function PlanosUsuarioPage() {
        ...
        <div className="mx-auto max-w-5xl px-4 py-10">
          ...
          <div className="mb-8">
            ...
            <div className="mt-4 text-center">
              ...
              <p className="mt-3 text-slate-500">Cuide melhor do seu pet com todas as funcionalidades</p>
            </div>
          </div>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900">Gratuito</h3>
                  <p className="text-sm font-bold text-slate-500">Para sempre</p>
                </div>
              </div>
              ...
              <ul className="mt-6 space-y-3">
                {funcionalidades.filter(f => f.gratis).map((f) => (
                  <li key={f.nome} className="flex items-center gap-2 text-sm text-slate-700">
              ...
              <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-center">
                <p className="text-sm font-semibold text-slate-600">Seu plano atual</p>
              </div>
            </div>
                ...
                {funcionalidades.map((f) => (
                  <li key={f.nome} className="flex items-center gap-2 text-sm text-slate-700">
                    <span className={`flex h-5 w-5 items-center justify-center rounded-full text-xs ${
                      f.premium ? 'bg-violet-100 text-violet-600' : 'bg-slate-100 text-slate-400'
                    }`}>
                      {f.premium ? '✓' : '✓'}
                    </span>
                    {f.nome}
          ...
          <section className="mt-16">
            <h2 className="text-center text-2xl font-black text-slate-900">Comparação completa</h2>
            <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="px-6 py-4 text-left text-sm font-bold text-slate-600">Funcionalidade</th>
                    <th className="px-6 py-4 text-center text-sm font-bold text-slate-600">Gratuito</th>
                    <th className="px-6 py-4 text-center text-sm font-bold text-violet-600">Premium</th>
                  </tr>
                </thead>
                <tbody>
                  {funcionalidades.map((f, i) => (
                    <tr key={f.nome} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      <td className="px-6 py-3 text-sm text-slate-700">{f.nome}</td>
                      <td className="px-6 py-3 text-center">
                        <span className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs ${
                          f.gratis ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'
                        }`}>
                          {f.gratis ? '✓' : '✕'}
                        </span>
                      </td>
          ...
          {/* Depoimentos */}
          <section className="mt-16">
            <h2 className="text-center text-2xl font-black text-slate-900">O que dizem nossos usuários</h2>
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              {[
                { nome: 'Ana Clara', pet: 'Luna, Golden', texto: 'O plano alimentar personalizado mudou a vida da minha Luna. Ela está mais saudável e feliz!', avatar: '👩' },
                { nome: 'Pedro Santos', pet: 'Max, Labrador', texto: 'Os alertas de vacinas são incríveis. Nunca mais esqueço de uma consulta importante.', avatar: '👨' },
                { nome: 'Maria Oliveira', pet: 'Thor e Mel', texto: 'Ter dois pets e poder comparar o desenvolvimento de cada um é fantástico!', avatar: '👩‍🦱' },
              ].map((d) => (
                <div key={d.nome} className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-4 flex items-center gap-3 border-t border-slate-100 pt-4">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-100 to-purple-100 text-xl">
                      {d.avatar}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-900">{d.nome}</p>
                      <p className="text-xs text-slate-500">{d.pet}</p>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {[
                {
                  ...
                  resposta: 'Sim! Não há multa ou taxa de cancelamento. Você pode cancelar a qualquer momento e continuar usando o Premium até o final do período pago.',
                },
                {
                  pergunta: 'O que acontece com meus dados se eu cancelar?',
                  resposta: 'Seus dados permanecem seguros. Você volta a ter acesso ao plano gratuito e pode assinar o Premium novamente quando quiser.',
                },
                {
                  pergunta: 'Posso usar em vários dispositivos?',
                  resposta: 'Sim! Sua conta é sincronizada na nuvem. Use no celular, tablet ou computador, seus dados sempre estarão atualizados.',
                },
                {
                  pergunta: 'Tem período de teste gratuito?',
                  resposta: 'Sim! Oferecemos 7 dias de teste gratuito do Premium. Cancele antes e não será cobrado nada.',
                },
              ].map((faq) => (
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl bg-white p-8 text-center shadow-2xl">
            ...
            <h2 className="mt-4 text-2xl font-black text-slate-900">Parabens!</h2>
            <p className="mt-2 text-slate-600">Voce agora e <strong className="text-violet-600">Premium</strong>! Cadastre quantos pets quiser.</p>
            <button
